...
//set permissions on page
int
set_page_perm(pde_t *pgdir, void *la, int pages, int perm) {
    for (int i = 0; i < pages; i++) {
      char *a = PGROUNDDOWN(la) + i * PGSIZE;
      cprintf("setting %x perms to %d\n", a, perm);
      pte_t *pte = walkpgdir(pgdir, a, 0);
      //page must be found
      if (pte == 0) {
        return -1;
      }
      //page must be present (mapped)
      if ((*pte & PTE_P) == 0) {
        return -1;
      }
